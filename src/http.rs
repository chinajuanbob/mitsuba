...
use governor::{clock::QuantaClock, state::keyed::DashMapStateStore, Jitter, Quota, RateLimiter};
...
use crate::util::{get_file_folder, get_file_url, get_host_string, get_proxy_config, hash_file};
...
impl HttpClient {
    pub fn new(
        ...
        HttpClient {
            limiter: Arc::new(RateLimiter::dashmap(
                Quota::per_minute(quota).allow_burst(burst),
            )),
            jitter: Arc::new(Jitter::new(
                Duration::from_millis(jitter_min),
                Duration::from_millis(jitter_interval),
            )),
            max_time,
            rclient,
    ...
    async fn fetch_url_bytes(
        ...
        attempt: u64,
        rlimit_key: &String,
    ) -> Result<bytes::Bytes, backoff::Error<reqwest::Error>> {
        self.limiter
            .until_key_ready_with_jitter(rlimit_key, *self.jitter)
            .await; // wait for rate limiter
    ...
    pub async fn fetch_url_backoff(
        &self,
        url: &str,
        rlimit_key: &String,
    ...
    pub async fn fetch_json<T: DeserializeOwned>(&self, url: &str) -> Result<T, bool> {
        let bytes = self
            .fetch_url_backoff(url, &"api".to_string())
            .await
            .map_err(|e| e.status().unwrap_or(StatusCode::OK) == StatusCode::NOT_FOUND)?;

        let obj = serde_json::from_slice(&bytes).map_err(|e| {
            error!("Failed to deserialize {} Error: {}", url, e);
            false
        })?;
        Ok(obj)
    }
    ...
    async fn upload_file(
        ...
        if let Some((_, code)) = self
            .oclient
            .bucket
            .put_object(filename.clone(), &bytes)
            .await
            .map_err(|e| {
                error!(
            ...
            .ok()
        {
            ...
            error!(
                "Error response code from object storage after upload request ({}): {}",
                filename, code
            );
        }
        None
    }
    pub async fn download_file_checksum(
        &self,
        url: &String,
        ext: &String,
        is_thumb: bool,
